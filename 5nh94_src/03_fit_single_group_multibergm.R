...
DATADIR <- "~/Documents/PhD/fMRI-paper/Data/"

# Get networks
net_ind <- 1:100
nets    <- readRDS(paste0("Data/n200_", thresh, ".RDS"))[net_ind]

print(paste("Running threshold", thresh))

# Set up ERGM formula and constraints
if (substr(thresh,1,1) == 'p') {
  ergm_formula <- nets ~ gwesp(0.75, fixed=TRUE) + gwnsp(0.75, fixed=T)
  constraints  <- ~edges
} else {
  ergm_formula <- nets ~ edges + gwesp(0.75, fixed=TRUE) + gwnsp(0.75, fixed=T)
  constraints  <- ~.
}
...
init          <- list()
init$theta    <- array(sweep(singles_mean, 2, pop_mean), 
                       c(length(nets), n_terms))
init$muPop    <- pop_mean
...
out_file <- file.path("Output", thresh, "young.RDS")
